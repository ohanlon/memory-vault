...
describe("openOrCreateDailyNote", () => {
  ...
  it("creates the folder and a scaffolded note on first use", () => {
    const result = openOrCreateDailyNote(root, "en-US", DATE);
    expect(result.created).toBe(true);
    expect(result.path).toBe(path.join(root, "daily", "08-27-2026.md"));
  ...
  it("opens the existing note instead of overwriting it on a second call", () => {
    const first = openOrCreateDailyNote(root, "en-US", DATE);
    fs.appendFileSync(first.path, "\nmy notes for today");

    const second = openOrCreateDailyNote(root, "en-US", DATE);
  ...
  it("uses the given locale's date ordering in the filename", () => {
    const result = openOrCreateDailyNote(root, "en-GB", DATE);
    expect(result.path).toBe(path.join(root, "daily", "27-08-2026.md"));
  });
});
