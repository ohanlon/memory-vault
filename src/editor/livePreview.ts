...
function processLine(
  ...
  // Strikethrough — processed before subscript below; see the note by STRIKETHROUGH_RE.
  for (const m of lineText.matchAll(STRIKETHROUGH_RE)) {
    const s = m.index!;
    const e = s + m[0].length;
    if (!isFree(s, e)) continue;
    markConsumed(s, e);
  ...
  // Subscript
  for (const m of lineText.matchAll(SUBSCRIPT_RE)) {
    const s = m.index!;
    const e = s + m[0].length;
    if (!isFree(s, e)) continue;
    markConsumed(s, e);
